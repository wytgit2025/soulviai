...
**为什么要两层**：`soulctl.py` 用系统自带 python3 就能跑（纯标准库）；真正的引擎必须用项目的解释器，且工作目录要是**数据家目录**（默认 `~/.soul-skill`，见 `engine/core/paths.py`）——数据全靠相对路径 `data/...` 解析。这个切换由 `bootstrap()` 自动完成，调用方只管把代码根（`engine/`）告诉它。
...
- 技能内**不含** `data/`；记忆写入数据家目录（默认 `~/.soul-skill`），引擎自动切换工作目录，不可逆。

...
description: |
  本技能通过 MCP 工具（soul_chat / soul_state / soul_pending / soul_drain …）调用本机的
...
# soul-skill · 数字生命服务（MCP 接入）

你已经接上了本机的数字生命引擎。**优先用下面这些 `soul_*` 工具**，不要自己去拼
`curl` / `python soulctl.py` 命令 —— 工具层已经处理好常驻优先、冷启动、鉴权和超时。
...
2. **多段要分段。** `soul_chat` 返回的**每个 content 块就是 ta 一条条发出来的消息**，按顺序分条呈现，不要拼成一大段。
...
5. **不要绕过它。** 不要直接读改 `engine/data/` 下的文件来制造对话，所有交互都经过 `soul_chat`。
6. **每一句话都是不可逆的相处。** `soul_chat` 会真实写入 ta 的记忆与情绪（陪伴积累暖意、敷衍积累隔阂）。不要拿无意义内容刷它。
...
| 场景 | 工具 |
|---|---|
| 用户说「跟 ta 说：…」 | `soul_chat(text=用户原话)` |
| 用户问「ta 怎么样 / 心情如何」 | `soul_state()`，再用 `codes.*`（`personality_stage` / `life_phase`）**转述成人话** |
| 想知道 ta 有没有主动想说的话 | `soul_pending(limit=10)` 先看，再 `soul_drain(limit=5)` 取 |
| 定时任务里把 ta 的话带给用户 | `soul_drain(limit=5)`，把每条 `text` 转达给用户，**不要再加解释** |
| 让 ta 的内心动一动 | `soul_tick()`，有情绪累积时会产生 1–3 条主动消息（`soul_drain` 取） |
| 首次使用 / 新建身份 | `soul_init()` |
| 工具报错、连不上 | `soul_health()` → `soul_doctor()` |
| 想更快（首次对话慢） | `soul_serve()` 常驻到内存 |
...
| `soul_ack` | 确认消息已送达（配 `soul_drain(peek=true)` 用） |
| `soul_tick` | 手动推进一次内心活动 |
| `soul_init` | 唤醒 / 初始化身份 |
| `soul_health` | 轻量体检：项目、解释器、常驻服务、token |
| `soul_serve` | 把引擎常驻到内存（含自主思考引擎） |
| `soul_stop` | 停掉常驻服务 |
| `soul_doctor` | 完整体检（`check_api=true` 会真打一次模型接口） |
...
**要不要带环境上下文**：`soul_chat` 支持 `env` / `env_json`，由**你自己**查好天气再传进去
（引擎不联网）。例如 `env_json='{"city":"上海","temperature":24,"is_raining":true}'`。
不传也没关系，ta 只是少了这层感知，对话完全不受影响。

**延迟是设计的一部分**：`soul_chat` 通常 3–20 秒返回，模型慢时可能 1 分钟以上。
...
- 终端直接用：`./run.sh`（独立运行版）
- 命令行 / Agent 用 bash 调：`python3 scripts/soulctl.py chat --text "…" --plain`（skill 版）
- 只有文档、服务已在别处跑：纯 md 版
...
| 现象 | 处理 |
|---|---|
| 工具报 `no_project` / `no_python` | 用 `soul_health()` 看诊断；缺解释器就跑 `soul_setup()` |
| 老是 `silent` | 用 `soul_doctor(check_api=true)` 确认真实原因，别当成 ta 在闹脾气 |
| 工具超时 | 首次冷启动或模型很慢；先 `soul_serve()` 常驻，或稍后重试 |
| token 不匹配 | 有别的副本起了服务：`soul_stop()` 后重启，或换 `daemon_port` |
| 想验证链路但不想花额度 | `soul_selftest()`（沙箱 + 假回复，不碰真实记忆） |
